...
public class ViewUtils {
	...
	public static void initializeMargin(Context ctx, View root, float dimension) {
		final int margin = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, dimension, ctx.getResources()
						.getDisplayMetrics());

		root.setPadding(margin, margin, margin, margin);

		FrameLayout frameLayout = new FrameLayout(ctx);
		frameLayout.setBackgroundColor(ctx.getResources().getColor(
				android.R.color.transparent));
		((ViewGroup) root).addView(frameLayout, new ViewGroup.LayoutParams(
